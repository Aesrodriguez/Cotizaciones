...
function EditablePrice({ value, onSave }: { value: number; onSave: (v: number) => Promise<void> }) {
...
function APUDetail({ apu, onPriceUpdated }: { apu: APUItem; onPriceUpdated: () => void }) {
  const hasContent = (apu.materiales?.length ?? 0) + (apu.mano_obra?.length ?? 0) + (apu.equipos?.length ?? 0) > 0
  if (!hasContent) return <p className="text-xs px-2 py-2" style={{ color: 'var(--text-muted)' }}>Sin detalle de insumos.</p>
              ...
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td className="px-3 py-1.5 text-right">
                  <EditablePrice
                    ...
                    onSave={async (v) => { await updateFn(r.id, { precio_unitario: v }); toast.success('Precio actualizado'); onPriceUpdated() }}
                  />
                </td>
                <td className="px-3 py-1.5 font-mono text-right" style={{ color: 'var(--text)' }}>{fmt(Number(r.subtotal ?? 0))}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function APUPage() {
  ...
  const toggleExpand = async (item: APUItem) => {
    if (expandedId === item.id) { setExpandedId(null); setExpandedData(null); return }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Master Pack Colombia 2026</p>
      </div>

      {loadError && (
        <div className="card text-center py-16">
          <p className="text-lg font-semibold mb-2" style={{ color: 'var(--text)' }}>No se pudo cargar</p>
          <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>Verifica tu conexión e intenta de nuevo.</p>
          <button onClick={loadCapitulos} className="btn-primary">Reintentar</button>
        </div>
      )}

      <div className={`flex gap-4 items-start ${capitulos.length === 0 ? 'hidden' : ''}`}>
        {/* Sidebar capítulos */}
        <div className="w-64 flex-shrink-0 rounded-xl overflow-hidden" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
              ...
              <div>
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5 text-right">
                            <EditablePrice
                              ...
                              onSave={async (v) => { await apuAPI.updatePrecio(item.id, v); toast.success('Precio actualizado'); loadItems() }}
                            />
                          </td>
                          ...
                          <tr key={`${item.id}-detail`} style={{ borderBottom: '1px solid var(--border)' }}>
                            <td colSpan={5} className="px-0 py-0">
                              {loadingDetail
                                ...
                                : expandedData
                                  ? <APUDetail apu={expandedData} onPriceUpdated={() => apuAPI.getById(item.id).then((r) => setExpandedData(r.data))} />
                                  : null}
                            </td>
                          </tr>
                        )}
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="btn-secondary text-xs py-1 px-3 disabled:opacity-40">Siguiente →</button>
                  </div>
                )}
              </div>
            )}
